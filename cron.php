...
$arrays = $db->select("(kicked = 0 AND cron=1) AND (updated_at <= DATE_SUB(NOW(), INTERVAL 10 HOUR) OR (`updated_at` IS NULL AND created_at <= DATE_SUB(NOW(), INTERVAL 10 HOUR)))", 'users', '15'); //1 - yuborilgani
